...
from .models import Quote, Model3D, Technology, Material, Bulk_Files, Quote_Form
...
def index(request):
    ...
    techno_list = []
    for filename in os.listdir('content/technos/'):
        if filename.rsplit(".",1)[1]=="yaml":
            techno = yaml.load(file("content/technos/%s"%filename, 'r'))
...
def quick_quote1(request):
    ...
    context = {'section':section,'techno':techno,'service':service,'request_success':request_success}
...
def technologies(request):
    ...
    techno_requested = request.GET.get('techno','')
    if techno_requested:
        techno = yaml.load(file("content/technos/%s.yaml"%techno_requested, 'r'))
    else:
        techno = ""

    # get all technos in list:
    techno_list = []
    for filename in sorted(os.listdir('content/technos/')):
        if filename.rsplit(".",1)[1]=="yaml":
            techno1 = yaml.load(file("content/technos/%s"%filename, 'r'))
            techno_list.append(techno1)
    # techno = yaml.load(urllib2.urlopen(S3_BUCKET + "media/private/content/technos/sls.yaml"))
...
def send_quote_form(request):
    ...
    if request.method == 'POST':
        ...
        new_quote_request = Quote_Form.objects.create(email=email, request_type = request_type, service=service, quantity=quantity, process=process, timeline=timeline, details=details, id_bulk_files=files_list_string)

        # slack message
        header = "New Request from Quotation Website :muscle:"

        if request_type == "prototype":
            message = "New request from: %s\nrequest type:%s\nprocess: %s\nquantity:%s\ntimeline: %s\ndetails: %s\nfiles: %s"%(email, request_type, process, quantity, timeline, details, slack_file_links)
        elif request_type == "service":
            message = "New request from: %s\nrequest type:%s\nservice: %s\nquantity:%s\ntimeline: %s\ndetails: %s\nfiles: %s"%(email, request_type, service, quantity, timeline, details, slack_file_links)
        elif request_type == "contact":
            message = "New request from: %s\nrequest type:%s\ndetails: %s"%(email, request_type, details)
        ...
        send_email(
            ...
            'contact@sp3d.co',
            ['thibault.de-saint-sernin@sp3d.co', 'contact@sp3d.co'],
            )
...
def dcv_verif(request):
    return HttpResponse(open('FDDB27C11433508C51F4F9AAEC7DEB01.txt').read())
